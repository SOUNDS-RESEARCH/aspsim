...
def concentricalCircles(numPoints, numCircles, radius, angleOffset="random", zDistance=None):
    ...
    coords = [equiangularCircle(circlePoints, radius, angle, zVal) 
                for i, (circlePoints, angle, zVal)
                in enumerate(zip(pointsPerCircle, startAngles, zValues))]
...
def uniformCylinder(numPoints, radius, height):
    ...
    for n in range(numPlanes):
        xyPoints = sunflowerPattern(pointsPerPlane, radius, np.random.rand()*2*np.pi)
...
def sunflowerPattern(N, radius, offsetAngle=0):
    phisq = np.square((np.sqrt(5)+1)/2);          # golden ratio
    k = np.arange(1,N+1)
...
def uniformDisc(pointDistance, radius, zAxis=None):
    ...
    if zAxis is not None:
        coordsCircle = np.concatenate((coordsCircle, np.full((coordsCircle.shape[0],1),zAxis)), axis=-1)
    return coordsCircle
...
def uniformFilledRectangle(numPoints, lim=(-2.4,2.4), zAxis=None):
    ...
    if zAxis is not None:
        evalPoints = np.concatenate((evalPoints, np.full((pointsPerAxis**2,1),zAxis)), axis=-1)
    return evalPoints
...
def FourEquidistantRectangles(numPoints, sideLength, sideOffset, zLow, zHigh, offset="distributed"):
...
def stackedEquidistantRectangles(numPoints, numRect, dims, zDistance, offset="distributed"):
    a = numRect // 2
    ...
    for i in range(numRect):
        points[idxCount:idxCount+pointsPerRect[i],0:2] = equidistantRectangle(pointsPerRect[i], dims, offset=offsets[i])
...
def stackedEquidistantRectangles_old(numPoints, numRect, dims, zDistance):
    ...
    points = np.zeros((numPoints, 3))
    for i in range(numRect):
        points[i*pointsPerRect[i]:(i+1)*pointsPerRect[i],0:2] = equidistantRectangle(pointsPerRect[i], dims)
...
def equidistantRectangle_old(numPoints, dims):
    ...
    if numPoints < 4:
        ...
        xVal = [-dims[0]/2, dims[0]/2, dims[0]/2,-dims[0]/2]
        yVal = [-dims[1]/2, -dims[1]/2, dims[1]/2,dims[1]/2]
        startPos = pointDist/2 + np.clip(np.random.randn()*0.3, -1, 1)*(pointDist/2)
        xFac = [1, 0, -1, 0]
        yFac = [0, 1, 0, -1]
        numCounter = 0
