...
class MPC_FF_postErrorNorm(AdaptiveFilterFF):
    ...
    def updateFilter(self):
        ...
        for n in range(self.updateIdx, self.idx):
            ...
            norm = np.sum(grad**2) / (self.beta + np.sum(np.sum(grad[:,:,None,:] * Xf, axis=(0,1,3))**2))
...
class MPC_FF_OptimalStepSize3(AdaptiveFilterFF):
    ...
    def updateSigmax(self):
        if self.sigmax is None:
            self.sigmax = np.mean(self.x[:,self.updateIdx+1:self.idx+1]**2, axis=(-1))
        else:
            self.sigmax *= self.forgetFactor
            self.sigmax += (1-self.forgetFactor) * np.mean(self.x[:,self.updateIdx+1:self.idx+1]**2, axis=-1)
