...
def test_directional_microphones_are_correct_with_regards_to_reciprocity():
    ...
    ref1_azimuth = -np.arctan2(
        (y1 + y2), (np.abs(pos_src[0, 0] - pos_mic[0, 0]))
    )  # correct for bottom walls

    y1 = wall_top - pos_mic[0, 1]
    y2 = wall_top - pos_src[0, 1]
    ref2_azimuth = np.arctan2(
        (y1 + y2), (np.abs(pos_src[0, 0] - pos_mic[0, 0]))
    )  # correct for top walls

    mic_dir = gp.spherical2cart(
        np.ones(
            1,
        ),
        np.array([[src_azimuth, np.pi / 2]]),
    )

    ir_omni = rir.ir_room_image_source_3d(
        pos_src,
        pos_mic,
        room_size,
        room_center,
    ...
    ir_dir = rir.ir_room_image_source_3d(
        pos_src,
        pos_mic,
        room_size,
        room_center,
    ...
    ax.add_patch(
        patches.Rectangle(
            corner, width, height, edgecolor="k", facecolor="none", linewidth=2, alpha=1
        )
    )
    ...
    ax.plot(
        [pos_mic[0, 0], pos_mic[0, 0] + 3 * mic_dir[0, 0]],
        [pos_mic[0, 1], pos_mic[0, 1] + 3 * mic_dir[0, 1]],
        label="Mic dir",
        linestyle="--",
    )
    ...
    ax.set_xlim(
        [
            np.min((pos_mic[:, 0], pos_src[:, 0])) - margin,
            np.max((pos_mic[:, 0], pos_src[:, 0])) - margin,
        ]
    )
...
def test_cardioid_microphone_facing_plane_wave_gives_same_response_as_omni_microphone():
    ...
    pw_dir = pw_dir / np.linalg.norm(
        pw_dir
    )  # incoming from that direction, not propagation direction
    mic_dir = pw_dir
    ...
    setup.add_mics(
        "cardioid", pos_mic, directivity_type=["cardioid"], directivity_dir=mic_dir
    )
    ...
    axes[1].plot(
        sim.arrays.paths["src"]["cardioid"][0, 0, :]
        - sim.arrays.paths["src"]["omni"][0, 0, :]
    )
    plt.show()
...
def test_cardioid_microphone_response_compared_to_omni_for_plane_wave_pyroomacoustics():
    ...
    pw_dir = np.array(
        [
            np.cos(pw_azimuth) * np.sin(pw_colatitude),
            np.sin(pw_azimuth) * np.sin(pw_colatitude),
            np.cos(pw_colatitude),
        ]
    )

    src_distance = 500
    room_size = np.array(
        [3 * src_distance, 3 * src_distance, 3 * src_distance]
    )  # make room big enough
    room_center = room_size / 2
    pos_mic = room_center[:, np.newaxis]
    pos_src = (
        room_center + pw_dir * src_distance
    )  # Put source very far away to approximate plane wave
    ...
    dir_obj = pra.directivities.CardioidFamily(
        orientation=pra.directivities.DirectionVector(
            azimuth=pw_azimuth, colatitude=pw_colatitude, degrees=False
        ),
...
def test_cardioid_microphone_response_to_plane_wave_of_different_heights_pyroomacoustics():
    ...
    dir_obj = pra.directivities.CardioidFamily(
        orientation=pra.directivities.DirectionVector(
            azimuth=pw_azimuth, colatitude=pw_colatitude + 0.3 * np.pi, degrees=False
        ),
        pattern_enum=pra.directivities.DirectivityPattern.CARDIOID,
    )
    room = pra.ShoeBox(room_size, fs=samplerate, max_order=0, use_rand_ism=False)
...
def _generate_plane_wave_with_ism(sr, pw_dir, pos_mic=None, pos_dir=None, mic_dir=None):
    ...
    setup.add_mics(
        "cardioid",
        pos_dir,
        directivity_type=pos_dir.shape[0] * ["cardioid"],
        directivity_dir=mic_dir,
    )
    setup.add_controllable_source("src", pos_src)
    sim = setup.create_simulator()
    ...
    return (
        sim.arrays["omni"].pos,
        fpaths["src"]["omni"][..., 0],
        sim.arrays.paths["src"]["omni"],
        sim.arrays["cardioid"].pos,
        fpaths["src"]["cardioid"][..., 0],
        freqs,
        sim.sim_info,
    )
...
def test_differential_microphone_filter_gives_same_result_as_pyroomacoustics_for_approximate_plane_wave_sound_field():
    ...
    mic_direction = gp.spherical2cart(
        np.ones(num_rotations),
        np.stack((mic_angle, np.pi / 2 * np.ones(num_rotations)), axis=-1),
    )
    ...
    setup.add_mics(
        "cardioid",
        pos_mic_tiled,
        directivity_type=dir_type,
        directivity_dir=mic_direction,
    )
    setup.add_controllable_source("src", pos_src)
    ...
    filter_below = 200
    cardioid_signal = directional.differential_cardioid_microphone(
        signal_main,
        signal_secondary,
        diff_mic_distance,
        500,
    ...
    power_per_angle_pra = np.mean(
        np.abs(sim.arrays.paths["src"]["cardioid"][0, :, :]) ** 2, axis=-1
    )
    ...
    fig, ax = plt.subplots(1, 1)
    example_idx = 5
    mse_time_response = np.mean(
        np.abs(cardioid_signal - sim.arrays.paths["src"]["cardioid"][0, :, :]) ** 2
    ) / np.mean(np.abs(sim.arrays.paths["src"]["cardioid"][0, :, :]) ** 2)
    ax.set_title(f"Mean square error: {mse_time_response}")
    ax.set_xlabel("Time (samples)")
    ax.set_ylabel("Amplitude")
    ax.plot(
        sim.arrays.paths["src"]["cardioid"][0, example_idx, :], label="Pyroomacoustics"
    )
    ...
    freq_mask[-5:] = False
    mse = np.mean(
        np.abs(freq_response_pra[freq_mask] - freq_response_diff[freq_mask]) ** 2
    ) / np.mean(np.abs(freq_response_pra[freq_mask]) ** 2)
    ...
    axes[0].plot(
        freqs,
        np.abs(ft.rfft(sim.arrays.paths["src"]["omni"][0, 0, :])),
        label="Pra omni",
    )
