...
def integrateMp(func, pointGenerator, totNumSamples, totalVolume, numPerIter= 5, verbose=False):
    """Uses mcIntegrate but paralellized"""
    ncpu = mp.cpu_count()
    #ncpu = 2
    integrationSamples = int(np.ceil(totNumSamples / ncpu))
    intArgs = [[func for _ in range(ncpu)], 
...
def integrate(func, pointGenerator, totNumSamples, totalVolume, numPerIter=50, verbose=False):
...
def diagnostics(newVal, oldVal, blockIdx):
    ...
    print("Max Abs change: " + "{:2.12E}".format(maxAbsChange) + 
            "     Its Value: " + "{:2.12E}".format(maxAbsChangeVal) + 
            "     rel change: " + "{:2.12E}".format(maxAbsChangeRel) )
    ...
    print("Max rel change: " + "{:2.12E}".format(maxRelChange) + 
            "     Its Value: " + "{:2.12E}".format(maxRelChangeVal)+
            "     abs change " +  "{:2.12E}".format(maxRelChangeAbs))
    
    avgRelChange = np.mean(np.abs(relChange))
    avgAbsChange = np.mean(np.abs(change))
    print("Avg rel change: " + "{:2.12E}".format(avgRelChange) + 
            "     Avg abs change: " + "{:2.12E}".format(avgAbsChange))
