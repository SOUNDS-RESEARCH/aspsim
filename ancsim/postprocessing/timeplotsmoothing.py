...
def processFolder(fldr, params):
    ...
    for f in os.listdir(fldr):
        ...
        if (
            ...
            and f.endswith(".tsv")
            and not f.endswith("processed.tsv")
        ):
...
def smoothData(signalToProcess, smoothLen, decibel=True):
    ...
    smoothedArray = np.convolve(
        signalToProcess, np.ones(smoothLen) / smoothLen, mode="valid"
    )
...
if __name__ == "__main__":
    ...
    params = {"smoothing": 32, "downsampling": 16}
    smoothFiles("reduction_100000/", params)
