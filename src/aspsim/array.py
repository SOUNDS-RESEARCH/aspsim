...
class ArrayCollection:
    ...
    def __init__(self):
        ...
        # rir_all[src_name][mic_name] -> ndarray (or on-disk np.memmap) of shape
        # (num_updates, num_src, num_mic, ir_len). Populated in setup_ir for
        # every (src, mic) combo where at least one side is dynamic. These are
        # simulation intermediates: they are backed by temporary memmap files (see
        # _rir_cache_dir), excluded from the pickle, and can be deleted after the
        # simulation with cleanup_rir_cache().
        self.rir_all = {}
        # Folder holding the dynamic-RIR memmap files (set lazily in setup_ir), and
        # the base folder it should be created under (the simulation fig folder).
        self._rir_cache_dir = None
        self._rir_cache_base = None
    ...
    def __getstate__(self):
        """Return picklable state, excluding the large dynamic-RIR intermediates.

        ``rir_all`` holds the per-update RIRs as on-disk memmaps (potentially many GB)
        that are pure simulation intermediates -- nothing reads them after the
        simulation, so they are never pickled. ``path_generator`` holds a back-reference
        to this collection plus the sim_info and is regenerated by setup_ir, so it is
        dropped as well.

        Returns
        -------
        state : dict
            The instance __dict__ with rir_all emptied and path_generator removed.
        """
        state = self.__dict__.copy()
        state["rir_all"] = {}
        state["_rir_cache_dir"] = None
        state.pop("path_generator", None)
        return state

    def __setstate__(self, state):
        """Restore state, defaulting the dropped intermediates for older pickles.

        Parameters
        ----------
        state : dict
            The pickled state produced by __getstate__.
        """
        self.__dict__.update(state)
        if getattr(self, "rir_all", None) is None:
            self.rir_all = {}
        if not hasattr(self, "_rir_cache_dir"):
            self._rir_cache_dir = None
        if not hasattr(self, "_rir_cache_base"):
            self._rir_cache_base = None

    def cleanup_rir_cache(self):
        """Delete the on-disk memmap RIR cache created during setup_ir.

        The dynamic-array RIRs are simulation intermediates that are not needed once the
        simulation has produced the recorded signals, so the temporary memmap files can
        be removed. Safe to call multiple times and when no cache exists.
        """
        for mic_dict in getattr(self, "rir_all", {}).values():
            for arr in mic_dict.values():
        ...
        self.rir_all = {}
        gc.collect()
        cache_dir = getattr(self, "_rir_cache_dir", None)
        if cache_dir is not None:
            shutil.rmtree(cache_dir, ignore_errors=True)
            self._rir_cache_dir = None

    def _alloc_rir_memmap(self, src, mic, reverb, sim_info):
        """Allocate an on-disk memmap to hold the dynamic RIRs for one (src, mic) combo.

        The backing file lives in a ``rir_cache`` subfolder of the simulation folder
        (or a tempdir if no folder was given). The cache folder is created lazily on the
        first call.
        ...
        reverb : str
            The propagation type. Determines the impulse-response length:
            max_room_ir_length for "ism"/"random", else 1.
        sim_info : SimulatorInfo
            The simulation info object.

        Returns
        -------
        memmap : np.memmap of shape (num_updates, src.num, mic.num, ir_len)
            A writable memmap for the dynamic RIRs.
        """
        if self._rir_cache_dir is None:
            if self._rir_cache_base is None:
                self._rir_cache_dir = pathlib.Path(
                    tempfile.mkdtemp(prefix="aspsim_rir_")
                )
            else:
                self._rir_cache_dir = pathlib.Path(self._rir_cache_base) / "rir_cache"
                self._rir_cache_dir.mkdir(parents=True, exist_ok=True)

        num_updates = src.pos_all.shape[0] if src.dynamic else mic.pos_all.shape[0]
        ir_len = sim_info.max_room_ir_length if reverb in ("ism", "random") else 1
        mmap_path = self._rir_cache_dir / f"rir_{src.name}_{mic.name}.npy"
        return np.lib.format.open_memmap(
            mmap_path,
            mode="w+",
            dtype=np.float64,
            shape=(num_updates, src.num, mic.num, ir_len),
        )
    ...
    def setup_ir(self, sim_info: cfutil.SimulatorInfo, rir_dir=None):
        ...
        sim_info : SimulatorInfo
            The simulation info object
        rir_dir : str or pathlib.Path, optional
            Base folder under which to store the temporary dynamic-RIR memmap files
            (a ``rir_cache`` subfolder is created). If None, a system tempdir is used.
        """
        self.sim_info = sim_info
        self._rir_cache_base = rir_dir
        self._rir_cache_dir = None
        ...
        for src, mic in self.mic_src_combos():
            ...
            if reverb != "modified":
                if src.dynamic or mic.dynamic:
                    # Generate the per-update RIRs directly into an on-disk memmap so the
                    # full (num_updates, ...) array is never held in RAM. create_path
                    # fills it block by block; we keep only a single in-RAM RIR slice in
                    # self.paths for the propagator's initial state.
                    out = self._alloc_rir_memmap(src, mic, reverb, sim_info)
                    _, path_info = self.path_generator.create_path(
                        src, mic, reverb, sim_info, True, True, out=out
                    )
                    out.flush()
                    self.rir_all.setdefault(src.name, {})[mic.name] = out
                    self.paths[src.name][mic.name] = np.array(out[0])
                else:
    ...
    def update(self, glob_idx: int):
        ...
        for ar_name in changed_arrays:
            ...
            # np.array() materializes the current RIR into a contiguous in-RAM ndarray.
            # rir_all entries are on-disk memmaps, and the dynamic FilterSum (numba)
            # stores a reference to the array it is given; a single (num_src, num_mic,
            # ir_len) copy per update is negligible and keeps the memmap views out of it.
            if changed_ar.is_mic:
                for src in self.sources():
                    if src.name in self.rir_all and ar_name in self.rir_all[src.name]:
                        self.paths[src.name][ar_name] = np.array(
                            self.rir_all[src.name][ar_name][i]
                        )
            elif changed_ar.is_source:
                for mic in self.mics():
                    if ar_name in self.rir_all and mic.name in self.rir_all[ar_name]:
                        self.paths[ar_name][mic.name] = np.array(
                            self.rir_all[ar_name][mic.name][i]
                        )
            else:
