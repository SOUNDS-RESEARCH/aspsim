...
class WaveDomainFF(AdaptiveFilterFFComplex):
    def __init__(self, mu, beta, speakerFilters, blockSize, errorMicPos, speakerPos):
        ...
        self.Te, self.TeInv = wd.waveTransformCircularMicArray(errorMicPos, self.numHarm)
        self.Tl, self.TlInv = wd.waveTransformSpeakerArray(speakerPos, errorMicPos, self.numHarm, 2*blockSize)

        self.secPathEstimate = np.zeros((2*self.blockSize, self.numHarm, self.numHarm), dtype=np.complex128)
        self.H = np.zeros((2*self.blockSize, self.numHarm, self.numHarm), dtype=np.complex128)
    ...
    def updateFilter(self):
        assert (self.idx - self.updateIdx == self.blockSize)
        assert(self.updated == False)
        self.E = np.fft.fft(self.e[:,self.updateIdx-self.blockSize:self.idx] * self.window,axis=-1)
        self.X = np.fft.fft(self.x[:,self.updateIdx-self.blockSize:self.idx] * self.window,axis=-1)
    ...
    def forwardPassImplement(self, numSamples):
        ...
        self.X = np.fft.fft(self.x[:,self.idx-self.blockSize:self.idx+self.blockSize]*self.window,axis=-1).T[:,:,None]
        self.Xw = self.Te
