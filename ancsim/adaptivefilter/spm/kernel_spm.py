...
class FastBlockKIFxLMSSPMEriksson(FastBlockFxLMSSPMEriksson):
    ...
    def __init__(self, config,  mu, muSPM,speakerFilters, blockSize, kiFilt):
        ...
        self.buffers["kixf"] = np.zeros((self.numSpeaker, self.numRef,self.numError,self.simChunkSize+s.SIMBUFFER))
...
class KernelSPM6(FastBlockFxLMSSPMEriksson):
    ...
    def __init__(self, config, mu, muSPM,speakerFilters, blockSize, errorPos, kiFiltLen, kernelReg, mcPointGen, ipVolume, 
        ...
        kiFiltFir = getAKernelTimeDomain3d(errorPos, kiFiltLen, kernelReg, mcPointGen, 
                                            ipVolume, mcNumPoints, 2048, self.samplerate, self.c)
...
class FDKIFxLMSErikssonInterpolatedF(FDKIFxLMSEriksson):
    def __init__(self, config, mu, beta, speakerFilters, blockSize, muSPM, kernFilt):
        ...
        self.secPathEstimate = FastBlockWeightedNLMS(blockSize=blockSize, numIn=self.numSpeaker, 
                                                    numOut=self.numError, weightMatrix=kernFilt,
                                                    stepSize=muSPM, freqIndepNorm=False)
        self.diag.addNewDiagnostic("weightedsecpath", dia.ConstantEstimateNMSE(self.A @ self.G, self.endTimeStep, plotFrequency=self.plotFrequency))
...
class KernelSPM2(ConstrainedFastBlockFxLMS):
    ...
    def __init__(self, config, mu, muSPM, speakerRIR, blockSize, errorPos):
        ...
        self.auxNoiseSource = WhiteNoiseSource(power=3, numChannels=self.numSpeaker)
        self.secPathEstimate = FastBlockWeightedNLMS(blockSize=blockSize, numIn=self.numSpeaker, numOut=self.numError, 
    ...
    def updateSPM(self):
        V = np.fft.fft(np.concatenate((np.zeros((self.numSpeaker,self.blockSize)),
...
class KernelSPM3(FastBlockFxLMSSPMEriksson):
    ...
    def __init__(self, config, mu, muSPM, eta, speakerRIR, blockSize, errorPos, kiFiltLen):
        ...
        self.buffers["vIP"] = np.zeros((self.numSpeaker, self.numError, self.numError, self.simChunkSize+s.SIMBUFFER))
    ...
    def constructInterpolation(self, errorPos, kernelRegParam, kiFiltLen):
        ...
        kiParams = np.zeros((2*self.blockSize, self.numError, self.numError), dtype=np.complex128)
        ...
        for m in range(self.numError):
            kiFIR = soundfieldInterpolationFIR(errorPos[m:m+1,:], 
                                            ...
                                            kiFiltLen, kernelRegParam, self.kiFreqSamples, 
                                            self.spatialDims, self.samplerate, self.c)
...
class KernelSPM5(FastBlockFxLMSSPMEriksson):
    ...
    def __init__(self, config, mu, muSPM, eta, speakerRIR, blockSize, errorPos, kiFiltLen):
        ...
        self.eta = eta
        kernelReg = 1e-3
        self.kiFilt = FilterMD_IntBuffer(dataDims=self.numSpeaker, ir=self.constructInterpolation(errorPos, kernelReg, kiFiltLen))

        self.secPathEstimate = FilterSum_IntBuffer(numIn=self.numSpeaker, numOut=self.numError, irLen=blockSize)
        self.secPathEstimateIP = FilterSum_IntBuffer(numIn=self.numSpeaker, numOut=self.numError, irLen=blockSize)
        self.secPathEstimateMD = FilterMD_IntBuffer(dataDims=self.numRef, ir=self.secPathEstimate.ir)
        self.kiDelay = kiFiltLen // 2
        self.kiFreqSamples = np.max(1024, 4*self.filtLen)
...
class KernelSPM3_old(FreqAuxNoiseFxLMS2):
    ...
    def constructInterpolation(self, errorPos, kernelRegParam):
        ...
        kiParams = np.zeros((2*self.blockSize, self.numError, self.numError), dtype=np.complex128)
        for m in range(self.numError):
            kiParams[:,m:m+1,np.arange(self.numError)!=m] = soundfieldInterpolation(errorPos[m:m+1,:], 
                                            errorPos[np.arange(self.numError) != m,:], 
    ...
    def getIPGrad(self):
        ...
        error2 = np.zeros((2*self.blockSize, self.numError, self.numSpeaker),dtype=np.complex128)
        for m in range(self.numError):
