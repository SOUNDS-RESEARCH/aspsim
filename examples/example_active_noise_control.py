...
def main():
    """Run the active noise control example."""
    ...
    setup.add_controllable_source(
        "loudspeaker", gp.equidistant_rectangle(num_ls, side_lengths=(2, 2), z=0)
    )
    ...
    sim.diag.add_diagnostic(
        "loudspeaker_signal",
        dg.RecordSignal("loudspeaker", sim.sim_info, num_channels=num_ls),
    )
    sim.diag.add_diagnostic(
        "microphone_signal",
        dg.RecordSignal("error", sim.sim_info, num_channels=num_mics),
    )
