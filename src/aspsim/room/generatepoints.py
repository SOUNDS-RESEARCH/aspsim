...
def equidistant_rectangle(
    ...
    fixed_side = [
        ...
        y_extent[0],
        x_extent[0],
    ]  # side with index i has a fixed coordinate for x or y. this is that coordinate

    points = np.zeros((num_points, 2))
    breakpoints = np.cumsum(
        np.array(
            [0, side_lengths[0], side_lengths[1], side_lengths[0], side_lengths[1]]
        )
    )
    idxs = [
        np.logical_and(
            breakpoints[i] <= pos_on_circumference,
            pos_on_circumference < breakpoints[i + 1],
        )
        for i in range(4)
    ]
