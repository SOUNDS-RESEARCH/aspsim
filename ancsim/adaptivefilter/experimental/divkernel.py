...
class KernelMPC_9b(AdaptiveFilterFF):
    def __init__(self, config, mu, speakerRIR, kernelFilt):
        ...
        self.Htf = tf.Variable(
            tf.zeros(self.H.shape, dtype=tf.float64), dtype=tf.float64
        )

        spec = [
            tf.TensorSpec(shape=kernelFilt.shape, dtype=tf.float64),
            tf.TensorSpec(shape=(self.numError, None), dtype=tf.float64),
            tf.TensorSpec(
                shape=(self.numRef, self.numSpeaker, self.numError, None),
                dtype=tf.float64,
            ),
    ...
    def computeGradient(self, L, e, xf, idxMin, idxMax, M, mu):
        ...
        for n in range(idxMin, idxMax):
            ...
            xfIP = [
                tf.reduce_sum(
                    L[None, None, :, :]
                    * tf.reverse(xf[:, :, :, n - M - i : n + M + 1 - i], axis=(-1,)),
                    axis=-1,
                )
                for i in range(filtLen)
            ]
            xfIP = tf.stack(xfIP)
            ...
            grad = tf.reduce_sum(
                eIP[None, None, None, :] * xfIP * norm[None, None, None, :], axis=-1
            )
    ...
    def forwardPassImplement(
        ...
        for b in blockSizes:
            self.saveToBuffers(
                noiseAtError[:, numComputed : numComputed + b],
                noiseAtTarget[:, numComputed : numComputed + b],
            )
            ...
            self.e[:, self.idx : self.idx + b] = (
                noiseAtError[:, numComputed : numComputed + b]
                + yf
                + errorMicNoise[:, numComputed : numComputed + b]
            )

            tyf = self.secPathTargetFilt.process(self.y[:, self.idx : self.idx + b])
            self.eTarget[:, self.idx : self.idx + b] = (
                noiseAtTarget[:, numComputed : numComputed + b] + tyf
            )

            evyf = self.secPathEvalsFilt.process(self.y[:, self.idx : self.idx + b])
            self.eEvals[:, self.idx : self.idx + b] = (
                noiseAtEvals[:, numComputed : numComputed + b] + evyf
            )

            self.xf[:, :, :, self.idx : self.idx + b] = np.transpose(
                self.secPathXfFilt.process(self.x[:, self.idx : self.idx + b]),
                (2, 0, 1, 3),
            )
