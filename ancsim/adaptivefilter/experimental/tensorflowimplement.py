...
class AdaptiveFilter_tf(ABC):
    def __init__(self, mu, beta, speakerRIR):
        ...
        self.H = tf.Variable(
            tf.zeros((self.numRef, self.numSpeaker, self.filtLen), dtype=tf.float64),
            dtype=tf.float64,
        )
        ...
        self.pointNoiseSmoother = Filter_IntBuffer(
            ir=np.ones((self.outputSmoothing)), numIn=self.numError
        )
...
class MPC_FF_tf(AdaptiveFilter_tf):
    ...
    def updateFilter(self):
        for i in range(self.blockSize):
            Xf = tf.reverse(
                self.xf[:, :, :, self.idx - i - self.filtLen + 1 : self.idx - i + 1],
                axis=(-1,),
            )
            self.H.assign_add(
                self.mu
                * (
                    tf.reduce_sum(
                        Xf * self.e[None, None, :, self.idx - i, None], axis=(2,)
                    )
    ...
    def forwardPass(
        ...
        self.tfForwardPass(
            numSamples, noiseAtError, noiseAtRef, noiseAtTarget, errorMicNoise
        )
    ...
    def tfForwardPass(
        ...
        for _ in range(numSamples):
            ...
            self.xf[:, :, :, n].assign(
                tf.reduce_sum(
                    X[:, None, None, :] * self.secPathError[None, :, :, :], axis=(-1,)
                )
            )

            tY = tf.reverse(self.y[:, n - self.tJ + 1 : n + 1], axis=(-1,))
            tyf = tf.reduce_sum(tY[:, None, :] * self.secPathTarget, axis=(0, -1))[
                :, None
            ]
            ...
            self.y[:, self.idx].assign(
                tf.reduce_sum(X[:, None, :] * self.H, axis=(0, -1))
            )
