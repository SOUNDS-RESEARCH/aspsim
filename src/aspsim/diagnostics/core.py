...
class Logger:
    ...
    def export_this_idx(self):
        ...
        for dg_name, dg in self.diagnostics.items():
            if (
                dg.next_export() <= dg.save_at.saved_until
            ):  # all relevant data have been saved
    ...
    def export_single_diag(self, diag_name, fldr):
        ...
        for exp_func, exp_kwarg, pp in zip(exp_funcs, exp_kwargs, preproc):
            exp_func(
                ...
                fldr,
                pp,
                print_method=self.sim_info.plot_output,
                **exp_kwarg,
            )
...
class IntervalCounter:
    ...
    def from_frequency(cls, frequency, max_value, include_zero=False):
        ...
        return cls(
            zip(
                range(start_value - 1, max_value, frequency),
                range(start_value, max_value + 1, frequency),
            ),
            num_values,
        )
...
class IndexCounter:
    ...
    def __init__(self, idx_selection, tot_samples=None):
        ...
        try:
            ...
            self.idx_selection = iter(
                range(idx_selection, tot_samples + 1, idx_selection)
            )
...
class Diagnostic:
    ...
    def __init__(
        ...
        self.export_function = [
            type(self).export_functions[func_choice] for func_choice in export_func
        ]
...
class StateDiagnostic(Diagnostic):
    ...
    def __init__(
        ...
        super().__init__(
            sim_info,
            export_at,
            IntervalCounter.from_frequency(
                save_frequency, sim_info.tot_samples, include_zero=False
            ),
            export_func,
            keep_only_last_export,
            export_kwargs,
...
class InstantDiagnostic(Diagnostic):
    ...
    def __init__(
        ...
        if save_at is None:
            ...
            save_at = IntervalCounter.from_frequency(
                save_freq, sim_info.tot_samples, include_zero=False
            )
            export_at = save_freq
            ...
            if isinstance(save_at, (tuple, list, np.ndarray)):
                ...
                save_at = IntervalCounter.from_frequency(
                    save_at, sim_info.tot_samples, include_zero=False
                )
        super().__init__(
            sim_info,
            export_at,
