...
def fftWithTranspose(timeSignal, n=None, addEmptyDim=False):
    ...
    newAxisOrder = np.concatenate(
        ([timeSignal.ndim - 1], np.arange(timeSignal.ndim - 1))
    )
...
def correlateEuclidianFF(freqFilter, freqSignal):
    ...
    filteredSignal = (
        freqFilter.reshape(freqFilter.shape + (1,) * (freqSignal.ndim - 1))
        * freqSignal.reshape(
            freqSignal.shape[0:1] + (1,) * (freqFilter.ndim - 1) + freqSignal.shape[1:]
        ).conj()
    )
...
def convolveEuclidianFF(freqFilter, freqSignal):
    ...
    filteredSignal = freqFilter.reshape(
        freqFilter.shape + (1,) * (freqSignal.ndim - 1)
    ) * freqSignal.reshape(
        freqSignal.shape[0:1] + (1,) * (freqFilter.ndim - 1) + freqSignal.shape[1:]
    )
    filteredSignal = ifftWithTranspose(filteredSignal)
