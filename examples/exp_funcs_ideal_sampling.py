"""Ideal sampling experiment helpers."""
...
def generate_constant_speed_trajectory(
    ...
    """Generate a trajectory at approximately constant speed.

    Parameters
    ----------
    position : function
        Function that takes in time and returns position.
    velocity : function
        Function that takes in time and returns velocity.
    samplerate : int
        Sample rate of the trajectory.
    target_speed : float
        Desired speed of the trajectory in meters per second.
    num_samples : int
        Number of samples in the trajectory.
    tolerance : float
        Allowed relative deviation from the target speed.
    """
...
def pos_bifurcation(low, high, offset, pos, previous_pos, desired_val, tolerance):
    """Find a timestep that matches a target distance."""
    speed = -1000
...
class LissajousTrajectoryConstantSpeed:
    ...
    def __init__(self, amplitude, freq, center, samplerate, speed_factor, num_samples):
        """Initialize a constant-speed Lissajous trajectory."""
    ...
    def current_pos(self, time_idx):
        """Return position at a given sample index."""
...
class LissajousTrajectory:
    """Lissajous trajectory with parametric speed."""

    def __init__(self, amplitude, freq, center):
        """Initialize a Lissajous trajectory."""
        self.amplitude = amplitude
        self.freq = freq
        assert self.freq.shape == (1, 3)
        # self.period_len = period_len
    ...
    def current_pos(self, time_idx):
        """Return position at a given sample index."""
        return self.center + self.amplitude * np.cos(
            2 * np.pi * time_idx * self.freq + self.phase_offset
        )
...
def generate_signals_3d(
    ...
    """Generate 3D signals for the ideal sampling experiment."""
...
def load_npz(signal_paths):
    """Load multiple npz signals into a dict."""
    sig = {}
...
def get_signal_paths(fig_folder):
    """Return a mapping of signal names to npz paths."""
    signal_paths = {}
...
def load_session(fig_folder):
    """Load a saved session and related signals."""
