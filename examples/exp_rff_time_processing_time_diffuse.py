...
def main_mc(num_mc, fig_folder = None, simulated_data = True):
    ...
    info = {
        "seq_len" : seq_len,
        "samplerate" : sim_info.samplerate,
        "c" : sim_info.c
    }
...
def main(figure_folder, mc_idx, info, signals, pos, rir_eval_freq):
    ...
    sig_len_list = [info["seq_len"] * num_periods for num_periods in [2, 4, 8, 16, 32, 64, 128] if num_periods <= tot_num_periods]
    ...
    for sig_len in sig_len_list:
        ...
        for nb in num_basis_list:
            print(f"moving mic rff with {nb} basis functions and signal length {sig_len}")
            ...
            compiled = jax.jit(sfe_jax.krr_moving_mic_rff, static_argnames=["num_basis", "return_params"]).lower(sig_moving_temp, pos_moving_temp, pos_eval, signals["loudspeaker_moving"], info["samplerate"], info["c"], regularization_rff, num_basis=nb, key = key, return_params=False).compile()
            flops_rff[-1].append(compiled.cost_analysis()[0]['flops'])

            process_time_start = time()
            estimates[f"moving rff {nb} sig len {sig_len}"] = jax.block_until_ready(compiled(sig_moving_temp, pos_moving_temp, pos_eval, signals["loudspeaker_moving"], info["samplerate"], info["c"], regularization_rff, key=key))
    ...
    for sig_len in sig_len_list:
        ...
        compiled = jax.jit(sfe_jax.krr_moving_mic_diffuse, static_argnames=["return_params"]).lower(sig_moving_temp, pos_moving_temp, pos_eval, signals["loudspeaker_moving"], info["samplerate"], info["c"], regularization_mo, return_params=False).compile()
        flops_ki.append(compiled.cost_analysis()[0]['flops'])

        process_time_start = time()
        estimates[f"moving ki {sig_len}"] = jax.block_until_ready(compiled(sig_moving_temp, pos_moving_temp, pos_eval, signals["loudspeaker_moving"], info["samplerate"], info["c"], regularization_mo))
    ...
    for sig_len in sig_len_list:
        ...
        for nb in num_basis_list:
            ...
            mse_per_freq = np.mean(np.abs(est - rir_eval_freq)**2, axis=-1) / np.mean(np.abs(rir_eval_freq)**2, axis=-1)
    ...
    with open(figure_folder / "times_rff.json", "w") as f:
        json.dump({"times_rff" : times_rff, "sig_len_list" : sig_len_list, "num_basis_list" : num_basis_list}, f)
    ...
    with open(figure_folder / "flops_rff.json", "w") as f:
        json.dump({"flops_rff" : flops_rff, "sig_len_list" : sig_len_list, "num_basis_list" : num_basis_list}, f)
    ...
    ax.plot(sig_len_list[:num_ki_estimates], times_ki, label="KRR moving mic", marker="o")
    for i, nb in enumerate(num_basis_list):
        ax.plot(sig_len_list, times_rff[:, i], label=f"RFF moving mic {nb} basis", marker="o")
    ...
    fig, ax = plt.subplots(1,1)
    ax.plot(np.log10(sig_len_list[:num_ki_estimates]), np.log10(times_ki), label="KRR moving mic", marker="o")
    for i, nb in enumerate(num_basis_list):
        ax.plot(np.log10(sig_len_list), np.log10(times_rff[:, i]), label=f"RFF moving mic {nb} basis", marker="o")
    ...
    fig, ax = plt.subplots(1,1)
    ax.plot(np.log10(sig_len_list[:num_ki_estimates]), np.log10(flops_ki), label="KRR moving mic", marker="o")
    for i, nb in enumerate(num_basis_list):
        ax.plot(np.log10(sig_len_list), np.log10(flops_rff[:, i]), label=f"RFF moving mic {nb} basis", marker="o")
    ax.set_xlabel("Signal length log10 samples")
    ...
    if "image" in pos:
        ...
        pos_image = None
    aspplot.soundfield_estimation_comparison(pos_eval, estimates, np.copy(rir_eval_freq), freqs, figure_folder, output_method=OUTPUT_METHOD, pos_image=pos_image, num_examples = 4, remove_freqs_above = 480, remove_freqs_below=20)

    with open(figure_folder / "mse_db.json", "r") as f:
        mse_db = json.load(f)
    mse_db_ki = np.array([mse_db[f"moving ki {sl}"] for sl in sig_len_list[:num_ki_estimates]])
    mse_db_rff = np.array([[mse_db[f"moving rff {nb} sig len {sl}"] for sl in sig_len_list] for nb in num_basis_list])
    fig, ax = plt.subplots(1,1)
    ax.plot(sig_len_list[:num_ki_estimates], mse_db_ki, label="KRR moving mic", marker="o")
    for i, nb in enumerate(num_basis_list):
        ax.plot(sig_len_list, mse_db_rff[i,:], label=f"RFF moving mic {nb} basis", marker="o")
    ax.set_xlabel("Signal length")
    ...
    with open(figure_folder / "mse.json", "r") as f:
        mse = json.load(f)
    mse_ki = np.array([mse[f"moving ki {sl}"] for sl in sig_len_list[:num_ki_estimates]])
    mse_rff = np.array([[mse[f"moving rff {nb} sig len {sl}"] for sl in sig_len_list] for nb in num_basis_list])

    with open(figure_folder / "mse_rff.json", "w") as f:
        json.dump({"mse_rff" : mse_rff.T.tolist(), "sig_len_list" : sig_len_list, "num_basis_list" : num_basis_list}, f)
...
def make_mc_plots(fig_folder):
    ...
    for i, nb in enumerate(num_basis_list):
        ...
        ax.fill_between(sig_len_list, mean_time - std_time, mean_time + std_time, alpha=0.2)
    ...
    ax.fill_between(sig_len_list[:num_ki_sig_lens], mean_time_ki - std_time_ki, mean_time_ki + std_time_ki, alpha=0.2)
    ...
    ax.fill_between(sig_len_list[:num_ki_sig_lens], mean_mse_ki - std_mse_ki, mean_mse_ki + std_mse_ki, alpha=0.2)
    ...
    for i, nb in enumerate(num_basis_list):
        mean_mse = np.mean(mse_rff[..., i], axis=0)
        std_mse = np.std(mse_rff[..., i], axis=0)
        ax.plot(sig_len_list, 10 * np.log10(mean_mse), label=f"RFF {nb} basis", marker="o")
        ax.fill_between(sig_len_list, 10 * np.log10(np.maximum(mean_mse - std_mse, 1e-10)), 10*np.log10(mean_mse + std_mse), alpha=0.2)
    
    mean_mse_ki = np.mean(mse_ki, axis=0)
    std_mse_ki = np.std(mse_ki, axis=0)
    ax.plot(sig_len_list[:num_ki_sig_lens], 10 * np.log10(mean_mse_ki), label="KRR", marker="o")
    ax.fill_between(sig_len_list[:num_ki_sig_lens], 10 * np.log10(np.maximum(mean_mse_ki - std_mse_ki, 1e-10)), 10 * np.log10(mean_mse_ki + std_mse_ki), alpha=0.2)
    ...
    mse_per_freq_rff = np.array(mse_per_freq_rff) # (num_mc, num_sig_len, num_basis, num_freqs)
    ...
    for sg_idx, sg in enumerate(sig_len_list):
        fig, ax = plt.subplots(1,1)
        freqs = ft.get_real_freqs(extra_params["seq_len"], samplerate)#np.arange(mse_per_freq_rff.shape[-1])
        for i, nb in enumerate(num_basis_list):
