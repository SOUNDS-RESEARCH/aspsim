...
class KernelIP11_papernorm(FxLMS_FF):
    ...
    def updateFilter(self):
        ...
        self.buffers["eKern"][:, self.updateIdx : self.idx] = self.eKernelFilt.process(
            self.e[:, self.updateIdx : self.idx]
        )

        for n in range(self.updateIdx, self.idx):
            term1 = self.buffers["eKern"][None, None:, n, None] * np.flip(
                self.buffers["xf"][:, :, :, n - M - self.filtLen + 1 : n - M + 1],
                axis=-1,
            )
            term2 = (
                np.flip(
                    self.buffers["xKern"][:, :, :, n - self.filtLen + 1 : n + 1],
                    axis=-1,
                )
                * self.e[None, None, :, n - M, None]
            )
...
class KernelIP11_minimumdelay(AdaptiveFilterFF):
    ...
    def updateFilter(self):
        ...
        for n in range(self.updateIdx, self.idx):
            term1 = self.buffers["eKern"][None, None:, n, None] * np.flip(
                self.xf[:, :, :, n - self.M - self.filtLen + 1 : n - self.M + 1],
                axis=-1,
            )
            term2 = (
                np.flip(
                ...
                * self.e[None, None, :, n - self.M, None]
            )
            # norm = 1 / (np.sum(self.xf[:,:,:,self.updateIdx-M:self.idx-M]**2) + self.beta)
...
class KernelIP11(AdaptiveFilterFF):
    ...
    def updateFilter(self):
        ...
        for n in range(self.updateIdx, self.idx):
            term1 = self.buffers["eKern"][None, None:, n, None] * np.flip(
                self.xf[:, :, :, n - M - self.filtLen + 1 : n - M + 1], axis=-1
            )
            term2 = (
                np.flip(
                    self.buffers["xKern"][:, :, :, n - self.filtLen + 1 : n + 1],
            ...
            norm = 1 / (
                np.sum(self.xf[:, :, :, self.updateIdx - M : self.idx - M] ** 2)
                + self.beta
            )
...
class KernelIP_simpleFromCostNewNorm(AdaptiveFilterFF):
    ...
    def updateFilter(self):
        ...
        for n in range(self.updateIdx, self.idx):
            ...
            norm = 1 / (
                np.sum(self.xf[:, :, :, self.updateIdx : self.idx] ** 2) + self.beta
            )
            grad += np.sum((term1 + term2), axis=2) * norm
...
class KernelIP11_postErrorNorm(AdaptiveFilterFF):
    ...
    def updateFilter(self):
        ...
        for n in range(self.updateIdx, self.idx):
            ...
            P = np.sum(
                np.flip(self.xf[:, :, :, n - M - self.filtLen + 1 : n - M + 1], axis=-1)
                * currentGrad[:, :, None, :],
                axis=(0, 1, 3),
            )
