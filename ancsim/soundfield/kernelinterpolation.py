...
def soundfieldInterpolationFIR(toPoints, fromPoints, irLen, regParam, 
    ...
    freqFilter = soundfieldInterpolation(toPoints, fromPoints, numFreq, 
                                    regParam, spatialDims, samplerate, c)
...
def kernelInterpolationFR(errorMicPos, freq, 
    ...
    A = np.transpose(P.conj(),(0,2,1)) @ np.transpose(S.conj(),(0,2,1)) @ Gamma @ S @ P
    return A
...
def getIntegrableFunc2d(errorPos, filtLen, regParam, numFreqSamples,
    ...
    def B(r):
        distance = np.transpose(distfuncs.cdist(np.transpose(r,(1,0)), errorPos), (1,0))[None,:,:]
        kappa = special.j0(distance*waveNum)
        freqFunc = Kinv @ kappa
        ir = fd.tdFilterFromFreq(np.transpose(freqFunc,(1,2,0)), filtLen, method="window", window="hamming")
...
def getRtimedomainCylinder3d(errorPos, blockSize, samplerate, c):
    ...
    tdR = np.concatenate((tdR[:,:,-blockSize+1:,:], tdR[:,:,0:blockSize+1,:]), axis=2)
...
def getRMatrixCylinder3d(errorPos, numFreq, numMCSamples, radius, height,
    ...
    integralVal = np.sum(Kinv[:,:,:,None,None] * integralVal[:,:,None,:,:], axis=1)
    integralVal = np.sum(Kinv[None,None,:,:,:] * integralVal[:,:,:,:,None], axis=3)

    allFreqs = np.concatenate((integralVal, np.flip(integralVal[1:-1,:,:,:], axis=0)), axis=0)
    allFreqs = np.concatenate((allFreqs, np.flip(allFreqs[:,:,1:-1,:], axis=2)), axis=2)
    return allFreqs
...
def getTimeDomainARect3d_even(errorPos, filtLen, numMCSamples, targetDim, height,
    ...
    tdA = np.real(np.fft.ifft(A, axis=0))
    tdA = np.concatenate((tdA[-halfLen+1:,:,:], tdA[0:halfLen+1,:,:]), axis=0)
...
def kernel12TimeDomainIntegrableFunc(errorPos, filtLen, numFreq, kernelreg, samplerate, c):
    ...
    def kernelFilter(r):
        ...
        fdFilter = np.concatenate((fdFilter, np.flip(fdFilter[:,1:-1,:,:], axis=1)), axis=1)

        tdFilter = np.real(np.fft.ifft(fdFilter, axis=1))
        tdFilter = np.concatenate((tdFilter[:,-halfPreSumLen:,:,:], tdFilter[:,0:halfPreSumLen+1,:,:]), axis=1)
        ...
        for i in range(preSumLen):
            tempFilt = tdFilter[:,i:i+1,:,:] * np.transpose(tdFilter, (0,1,3,2))
            totOutputFilt[:,:,:,i:i+preSumLen] += np.transpose(tempFilt, (0,2,3,1))

        midPoint = totFiltLen // 2
        halfFiltLen = filtLen // 2
        outputFilt = totOutputFilt[:,:,:,midPoint-halfFiltLen:midPoint+halfFiltLen+1]
            ...
            for otherFiltAxis in range(maxLen):
                ...
                for i in range(midIdx, maxLen):
                    ...
                    if energyCount > energyNeeded:
                        reqFiltLen[0,otherFiltAxis,eSameAxis,eOtherAxis] = (i+1 - midIdx) * 2
                        break
    for eSameAxis in range(numError):
        for eOtherAxis in range(numError):
            for otherFiltAxis in range(maxLen):
                ...
                for i in range(midIdx, maxLen):
                    ...
                    if energyCount > energyNeeded:
                        reqFiltLen[1,otherFiltAxis,eSameAxis,eOtherAxis] = (i+1 - midIdx) * 2
                        break
    if maxLen % 2 == 1:
        reqFiltLen -= 1
