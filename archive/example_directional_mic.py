...
setup.add_mics(
    "mic",
    np.array([[0, 0, 0]]),
    directivity_type=["cardioid"],
    directivity_dir=directivity_dir,
)
...
for i in range(num_src):
    sim.diag.add_diagnostic(
        f"recorded_ls_sig_{i}",
        dg.RecordSignal(f"ls_{i}~mic", sim.sim_info, export_func="npz"),
    )
sim.diag.add_diagnostic(
    "microphone_signal", dg.RecordSignal("mic", sim.sim_info, export_func="npz")
)
...
for i in range(num_src):
    rec_sig = np.load(
        sim.folder_path.joinpath(f"recorded_ls_sig_{i}_{sim.sim_info.tot_samples}.npz")
    )
...
true_cardoid_power = (
    0.5 + 0.5 * np.sum(directivity_dir * pos_true_cardioid, axis=-1)
) ** 2

# Plot the results
fig, ax = plt.subplots(1, 1, figsize=(8, 6), subplot_kw={"projection": "polar"})
ax.plot(
    true_cardioid_angles,
    true_cardoid_power,
    label="True cardioid power",
    alpha=0.4,
    linewidth=5,
)
