...
class UnconstrainedFreqAuxNoiseFxLMS(ConstrainedFastBlockFxLMS):
    ...
    def updateSPM(self):
        self.E = np.fft.fft(self.e[:,self.updateIdx-self.blockSize:self.idx] * \
                            self.window, axis=-1).T[:,:,None]

        self.V = np.fft.fft(self.buffers["v"][:,self.updateIdx-self.blockSize:self.idx] * \
                            self.window, axis=-1).T[:,:,None]

        self.F = self.E - self.secPathEstimate.ir @ self.V #self.buffers["vEst"][:,self.updateIdx:self.idx]
