...
class KernelIP_FB(AdaptiveFilterFB):
    def __init__(self, config, mu, beta, speakerRIR, kernelFilt):
        ...
        spec = [
            ...
            tf.TensorSpec(
                shape=(self.numError, self.numSpeaker, self.numError, None),
                dtype=tf.float64,
            ),
    ...
    def updateFilter(self):
        ...
        if idxMin < idxMax:
            self.H = self.computeGradient(
                ...
                tf.convert_to_tensor(np.float64(self.beta)),
            ).numpy()
    ...
    def computeGradient(self, c, e, xf, idxMin, idxMax, M, mu, beta):
        ...
        for n in range(idxMin, idxMax):
            I = (
                tf.transpose(
                    tf.reverse(e[:, n - M : n + M + 1, None, None], axis=(1,)),
                    (1, 0, 2, 3),
                )
                * c
            )
            ...
            grad = [
                tf.nn.conv1d(
                    tf.transpose(
                        tf.reverse(
                            xf[u, :, :, n - M - filtLen + 1 : n + M + 1], axis=(-1,)
                        ),
                        (0, 2, 1),
                    ),
            ...
            P = tf.reduce_sum(
                grad[:, :, None, :]
                * tf.reverse(xf[:, :, :, n - filtLen + 1 : n + 1], axis=(-1,)),
                axis=(0, 1, 3),
            )
            norm = tf.reduce_sum(e[:, n] * P) / (
                tf.reduce_sum(tf.square(P)) + beta * 0.0001
            )
...
class KernelIP_FFavgnorm(AdaptiveFilterFF):
    ...
    def computeGradient(self, c, e, xf, idxMin, idxMax, M, mu, beta):
        ...
        for n in range(idxMin, idxMax):
            ...
            self.norm.assign_add(
                tf.reduce_sum(e[:, n] * P) / (tf.reduce_sum(tf.square(P)) + beta)
            )
        ...
        self.norm.assign(
            self.norm
            / (tf.cast(idxMax, dtype=tf.float64) - tf.cast(idxMin, dtype=tf.float64))
        )
