"""Active noise control processor examples."""
...
def add_anc_diagnostics(sim):
    """Attach standard ANC diagnostics to a simulator."""
    sim.diag.add_diagnostic(
        "noise_reduction",
        dia.SignalPowerRatio(
            "error", "noise~error", sim.sim_info, preprocess=pp.db_power
        ),
    )
    sim.diag.add_diagnostic(
        "power_ls",
        dia.SignalPower(
            "loudspeaker",
            sim.sim_info,
            preprocess=[[pp.smooth(sim.sim_info.output_smoothing), pp.db_power]],
        ),
    )
    sim.diag.add_diagnostic(
        "power_error",
    ...
    sim.diag.add_diagnostic(
        "sig_ls",
        dia.RecordSignal(
            "loudspeaker", sim.sim_info, num_channels=sim.arrays["loudspeaker"].num
        ),
    )
    sim.diag.add_diagnostic(
        "sig_error",
        dia.RecordSignal("error", sim.sim_info, num_channels=sim.arrays["error"].num),
    )
    sim.diag.add_diagnostic(
        "sig_ref",
        dia.RecordSignal("ref", sim.sim_info, num_channels=sim.arrays["ref"].num),
    )
    sim.diag.add_diagnostic(
        "sig_primary_noise",
        dia.RecordSignal(
            "noise~error", sim.sim_info, num_channels=sim.arrays["error"].num
        ),
    )
    sim.diag.add_diagnostic(
        "sig_secondary_noise",
        dia.RecordSignal(
            "loudspeaker~error", sim.sim_info, num_channels=sim.arrays["error"].num
        ),
    )

    sim.diag.add_diagnostic(
        "secondary_paths",
        dia.RecordFilter("arrays.paths['loudspeaker']['error']", sim.sim_info),
    )
    ...
    if "eval" in sim.arrays:
        sim.diag.add_diagnostic(
            ...
            dia.SignalPowerRatio(
                "eval", "noise~eval", sim.sim_info, preprocess=pp.db_power
            ),
        )


class ActiveNoiseControlProcessor(afbase.AudioProcessor):
    """Base class for ANC processors."""
    ...
    def __init__(self, sim_info, arrays, block_size, update_block_size, **kwargs):
        ...
        assert update_block_size % block_size == 0, (
            "Update block size must be a multiple of block size"
        )
        assert update_block_size >= block_size, (
            "Block size must be smaller or equal to update block size"
        )

        self.counter = utils.EventCounter(
            {"update_filter": (update_block_size, update_block_size)}
        )
    ...
    def gen_speaker_signals(self, num_samples):
        """Generate loudspeaker signals for a block."""
        pass

    @abstractmethod
    def update_filter(self):
        """Update the control filter."""
        pass


class TDANCProcessor(ActiveNoiseControlProcessor):
    """Base class for ANC with a time-domain control filter..
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.control_filter = fc.create_filter(
            num_in=self.num_ref, num_out=self.num_speaker, ir_len=self.filt_len
        )
    ...
    def gen_speaker_signals(self, num_samples):
        ...
        self.sig["loudspeaker"][:, self.sig.idx : self.sig.idx + num_samples] = (
            self.control_filter.process(
                self.sig["ref"][:, self.sig.idx - num_samples : self.sig.idx]
            )
        )
...
class FxLMS(TDANCProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            sim_info, arrays, block_size, update_block_size, control_filt_len, **kwargs
        )
        ...
        sec_path_ir = np.concatenate(
            (
                np.zeros((sec_path_ir.shape[0], sec_path_ir.shape[1], self.block_size)),
                sec_path_ir,
            ),
            axis=-1,
        )
        self.sec_path_filt = fc.create_filter(
            ir=sec_path_ir, broadcast_dim=self.num_ref, sum_over_input=False
        )
    ...
    def prepare(self):
        """Prepare secondary path filtering for processing."""
        super().prepare()
    ...
    def update_filter(self):
        """Update the control filter weights."""
        ...
        for n in range(start_idx, end_idx):
            Xf = np.flip(self.sig["xf"][..., n - self.filt_len + 1 : n + 1], axis=-1)
            grad_contribution = np.sum(
                Xf * self.sig["error"][None, None, :, n : n + 1], axis=2
            )
        ...
        if self.normalization == "block":
            norm = 1 / (
                np.sum(self.sig["xf"][:, :, :, start_idx:end_idx] ** 2) + self.beta
            )
            grad *= norm
...
class FastBlockFxLMS(TDANCProcessor):
    """Run a fully block-based FxLMS using overlap-save and FFT.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            sim_info, arrays, block_size, control_filt_len, control_filt_len, **kwargs
        )
        ...
        sec_path_ir = np.concatenate(
            (np.zeros((*sec_path.shape[:2], self.block_size)), sec_path), axis=-1
        )
            ...
            sec_path_ir = np.concatenate(
                (
                    ...
                    np.zeros(
                        (
                            *sec_path_ir.shape[:2],
                            2 * self.filt_len - sec_path_ir.shape[-1],
                        )
                    ),
                ),
                axis=-1,
        ...
        self.sec_path_filt = fc.FilterBroadcastFreq(
            data_dims=self.num_ref, tf=sec_path_tf
        )
        self.sig.create_signal("xf", (self.num_ref, self.num_speaker, self.num_error))
    ...
    def update_filter(self):
        ...
        self.sig["xf"][:, :, :, start_idx:end_idx] = np.moveaxis(
            self.sec_path_filt.process(self.sig["ref"][:, start_idx:end_idx]), 2, 0
        )
        ...
        norm = 1 / (np.sum(self.sig["xf"][:, :, :, start_idx:end_idx] ** 2) + self.beta)
        self.control_filter.ir -= self.mu * norm * td_grad


def equivalent_delay(ir):
    """Compute the equivalent delay for a single-channel IR.
