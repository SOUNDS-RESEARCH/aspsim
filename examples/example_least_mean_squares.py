"""Least mean squares example processors."""

raise NotImplementedError


class BlockLeastMeanSquares(AudioProcessor):
    """Block-based least mean squares processor."""
    ...
    def __init__(self, config, arrays, block_size, stepSize, beta, filtLen):
        ...
        self.controlFilter = fc.FilterSum(
            irLen=self.filtLen, numIn=self.numIn, numOut=self.numOut
        )

        self.diag.add_diagnostic(
            "inputPower",
            dia.SignalPower("input", self.sim_info.tot_samples, self.outputSmoothing),
        )
        self.diag.add_diagnostic(
            "desiredPower",
            dia.SignalPower("desired", self.sim_info.tot_samples, self.outputSmoothing),
        )
        self.diag.add_diagnostic(
            "errorPower",
            dia.SignalPower("error", self.sim_info.tot_samples, self.outputSmoothing),
        )

    def process(self, numSamples):
        """Process a block of samples."""
        self.sig["estimate"][:, self.idx - numSamples : self.idx] = (
            self.controlFilter.process(
                self.sig["input"][:, self.idx - numSamples : self.idx]
            )
        )

        self.sig["error"][:, self.idx - numSamples : self.idx] = (
            self.sig["desired"][:, self.idx - numSamples : self.idx]
            - self.sig["estimate"][:, self.idx - numSamples : self.idx]
        )
        ...
        for n in range(numSamples):
            grad += (
                np.flip(
                    self.sig["input"][
                        None, :, self.idx - self.filtLen - n : self.idx - n
                    ],
                    axis=-1,
                )
        ...
        normalization = 1 / (
            np.sum(self.sig["input"][:, self.idx - self.filtLen : self.idx] ** 2)
            + self.beta
        )
...
class LeastMeanSquares(AudioProcessor):
    """Conventional least mean squares processor.
    ...
    def __init__(self, sim_info, arrays, block_size, stepSize, beta, filtLen):
        ...
        self.diag.add_diagnostic(
            "inputPower", dia.SignalPower(self.sim_info, self.input)
        )
        self.diag.add_diagnostic(
            "desiredPower", dia.SignalPower(self.sim_info, self.desired)
        )
        self.diag.add_diagnostic(
            "errorPower", dia.SignalPower(self.sim_info, self.error)
        )
        self.diag.add_diagnostic(
            "paramError",
            dia.StateNMSE(
                ...
                "controlFilter.ir",
                np.pad(
                    self.arrays.paths["source"][self.desired],
                    ((0, 0), (0, 0), (0, 512)),
                    mode="constant",
                    constant_values=0,
                ),
    ...
    def process(self, numSamples):
        ...
        for i in range(self.idx - numSamples, self.idx):
            self.sig[self.estimate][:, i] = np.squeeze(
                self.controlFilter.process(self.sig[self.input][:, i : i + 1]), axis=-1
            )

            self.sig[self.error][:, i] = (
                self.sig[self.desired][:, i] - self.sig[self.estimate][:, i]
            )

            grad = (
                np.flip(
                    self.sig[self.input][:, None, i + 1 - self.filtLen : i + 1], axis=-1
                )
                * self.sig[self.error][None, :, i : i + 1]
            )

            normalization = 1 / (
                np.sum(self.sig[self.input][:, i + 1 - self.filtLen : i + 1] ** 2)
                + self.beta
            )
            self.controlFilter.ir += self.stepSize * normalization * grad
