...
def resample_multichannel(ir, ratio):
    ...
    for ls_idx in range(ir.shape[0]):
        ...
        for mic_idx in range(ir.shape[1]):
            ...
            ir_out[ls_idx].append(
                samplerate.resample(ir[ls_idx, mic_idx, ...], ratio, "sinc_fastest")
            )
...
def main():
    ...
    setup.sim_info.export_frequency = (
        setup.sim_info.tot_samples
    )  # only saves the data at the end of the simulation
    ...
    ir_eval = ir_eval[
        ..., int(0.1 * setup.sim_info.samplerate) :
    ]  # Remove the first 100 ms of IR since it is mostly silence
    ir_eval = ir_eval[
        ..., : setup.sim_info.max_room_ir_length
    ]  # Make the IR length correspond to the settings chosen earlier
    ...
    setup.add_free_source(
        "noise",
        np.array([[-3, 0, 0]]),
        src.BandlimitedNoiseSource(1, 1, (40, 300), setup.sim_info.samplerate, rng=rng),
    )

    setup.set_path("loudspeaker", "error", ir_mic)  # use real IRs
    setup.set_path("loudspeaker", "eval", ir_eval)  # use real IRs
    setup.arrays.path_type["loudspeaker"]["ref"] = (
        "none"  # no feedback from the loudspeakers to the reference microphones
    )
    setup.arrays.path_type["noise"]["ref"] = (
        "direct"  # reference signal is the noise signal directly
    )
    ...
    proc = anc_processors.FastBlockFxLMS(
        sim.sim_info,
        sim.arrays,
        block_size,
        control_filt_len,
