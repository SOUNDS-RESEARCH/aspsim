...
def differential_cardioid_microphone(
    signal_main,
    signal_secondary,
    microphone_distance,
    filter_order,
    ...
    num_freq = (
        4 * filter_order
    )  # we need more or equal frequency samples compared to the ir_len
    ...
    def freq_response_main_mic(f):
        ...
        wave_num[wave_num == 0] = (
            1e-10  # To avoid division by zero warnings. Not strictly necessary.
        )
    ...
    def freq_response_secondary_mic(f):
        ...
        both_mic_factor[0] = 0
        mic2_factor = -np.conj(
            both_mic_factor * np.exp(-1j * wave_num * microphone_distance)
        )
    ...
    mic1_ir = fd.fir_from_frequency_function(
        freq_response_main_mic, ir_len, samplerate, window=None
    )
    mic2_ir = fd.fir_from_frequency_function(
        freq_response_secondary_mic, ir_len, samplerate, window=None
    )
    ...
    signal_main_filtered = spsig.fftconvolve(
        mic1_ir[None, :], signal_main, axes=-1, mode="full"
    )
    signal_secondary_filtered = spsig.fftconvolve(
        mic2_ir[None, :], signal_secondary, axes=-1, mode="full"
    )

    cardioid_signal = signal_main_filtered + signal_secondary_filtered
    cardioid_signal = cardioid_signal[
        ..., filter_order : filter_order + signal_main.shape[-1]
    ]
