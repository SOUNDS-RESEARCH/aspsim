...
class RecordSignal(diacore.SignalDiagnostic):
    ...
    def save(self, processor, sig, chunk_interval, glob_interval):
        ...
        if self.channel_idx is not None:
            self.signal[:, glob_interval[0] : glob_interval[1]] = sig[self.sig_name][
                self.channel_idx, chunk_interval[0] : chunk_interval[1]
            ]
        else:
            self.signal[:, glob_interval[0] : glob_interval[1]] = sig[self.sig_name][
...
class RecordState(diacore.StateDiagnostic):
    ...
    def __init__(
        self, state_name, state_dim, *args, label_suffix_channel=None, **kwargs
    ):
        super().__init__(*args, **kwargs)
...
class SignalPowerRatio(diacore.SignalDiagnostic):
    ...
    def save(self, processor, sig, chunk_interval, glob_interval):
        ...
        smoother_denom = fc.create_filter(
            ir=np.ones((1, 1, self.sim_info.output_smoothing))
            / self.sim_info.output_smoothing
        )

        num = smoother_num.process(
            np.mean(
                np.abs(
                    sig[self.numerator_name][
                        self.numerator_channels, chunk_interval[0] : chunk_interval[1]
                    ]
                )
                ** 2,
                axis=0,
    ...
    for sig_name in processor.sig.keys():
        processor.diag.add_diagnostic(
            ...
            SignalPower(
                ...
                preprocess=[
                    [pp.smooth(processor.sim_info.output_smoothing), pp.db_power]
                ],
            ),
        )
