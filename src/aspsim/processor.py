...
class DebugProcessor(AudioProcessor):
    ...
    def process(self, num_samples):
        ...
        for manual_i, i in zip(
            range(self.manual_idx, self.manual_idx + num_samples),
            range(self.sig.idx - num_samples, self.sig.idx),
        ):
...
def block_process_until_index(earliest_start_index, index_to_end_at, block_size):
    ...
    for i in range(num_blocks):
        yield (
            index_to_start_at + i * block_size,
            index_to_start_at + (i + 1) * block_size,
        )
