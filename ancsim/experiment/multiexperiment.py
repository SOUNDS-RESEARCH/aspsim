            ...
            for dictIdx, singleDictParamNames in enumerate(self.paramNames):
                for paramName, paramValueIdx in zip(singleDictParamNames, allIndices):
                    if paramName is not None:
                        newDicts[dictIdx][paramName] = self.parameters[dictIdx][paramName][paramValueIdx]
            yield newDicts
...
class ExperimentSpec():
    ...
    def genExperimentSets(self):
        for config, filtArgs in self.betweenSets.generateParameters(self.baseConfig, self.baseFiltParameters):
            configList = []
            filtArgList = []
