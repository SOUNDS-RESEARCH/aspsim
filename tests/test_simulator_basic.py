...
def test_simulation_equals_direct_convolution_random_rirs(fig_folder):
    ...
    sig_sim = np.load(sim.folder_path.joinpath(f"mic_{sim.sim_info.tot_samples}.npz"))[
        "mic"
    ]
...
def test_simulation_equals_direct_convolution_random_rirs_multiple_sources(
    ...
    for s in range(num_src):
        setup.add_free_source(
            f"src_{s}", np.array([[1, 0, 0]]), sources.Sequence(src_sig[s])
        )
    ...
    filt = [
        fc.create_filter(ir=sim.arrays.paths[f"src_{s}"]["mic"]) for s in range(num_src)
    ]
    direct_mic_sig = np.sum(
        np.concatenate([filt[s].process(src_sig[s]) for s in range(num_src)], axis=0),
        axis=0,
    )
